...
function Convertor(props) {
    ...
    const [exRate, setExRate] = useState(props.rateData.rate ? props.rateData.rate : "");
    ...
    if (amountInFromCurrency) {
        fromAmount = amount;
        if (exRate) {
            toAmount = (amount / exRate).toFixed(2);
        } else {
            toAmount = 0;
        }
    } else {
        toAmount = amount;
        fromAmount = (amount * exRate).toFixed(2);
    }
    ...
    return (
        <div id="convertor">
            <Layout className="layout">
                ...
                <Footer style={{ textAlign: "center" }}>Lilian Design @2021 Currency Convertor</Footer>
            </Layout>
        </div>
    );
}
